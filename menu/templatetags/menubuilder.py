...
def build_menu(parser, token):
    """
    Build a menu. if the user is not logged in then only the items that
    do not require login are displayed.
    {% menu menu_name %}
    """
    try:
...
class MenuObject(template.Node):
    """
    This is the menu that the tag creates.
    """
...
def get_items(menu, current_path, user):
    """
    Get all the items in the menu that relate to a given path and user.
    """
    menuitems = []
